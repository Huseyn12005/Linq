...
////2)
//var deptor_borc = debtors.Where(debtor => (debtor.Email.EndsWith("rhyta.com")) || debtor.Email.EndsWith("dayrep.com"));

//foreach (var item in deptor_borc)
//{
...
//3)
//var deptor_yash = debtors.Where(debtor => (DateTime.Today.Year - debtor.BirthDay.Year > 26) && (2023 - debtor.BirthDay.Year < 36));

//foreach (var item in deptor_yash)
//{
//    Console.WriteLine(item);
//}
...
////4)
//var deptor_borc_say = debtors.Where(debtor => debtor.Debt <= 5000);

//foreach (var item in deptor_borc_say)
//{
//    Console.WriteLine(item);
//}
...
////5)
//var deptor = debtors.Where(debtor => (debtor.FullName.Length > 18) && (debtor.Phone.Count(number => number == '7') >= 2));
...
//var deptor_qis = debtors.Where(debtor => (debtor.BirthDay.Month == 12) || (debtor.BirthDay.Month == 1) || (debtor.BirthDay.Month == 2));

//foreach (var item in deptor_qis)
//{
//    Console.WriteLine(item);
//}
...
//double debt_cem = debtors.Average(debtor => debtor.Debt);

//var deptor1 = debtors.Where(debtor => debtor.Debt > debt_cem).ToList();


//var sortbyFullName = deptor1.OrderBy(debtor => debtor.FullName);
//foreach (var item in sortbyFullName)
//{
//    Console.WriteLine(item);
//}
...
//var sortbyDebt = deptor1.OrderBy(debtor => debtor.Debt);
//foreach (var item in sortbyFullName)
//{
//    Console.WriteLine(item);
//}
...
//var list = debtors.Where(debtor => !debtor.Phone.Contains('8')).ToList();
//foreach (var debtor in list)
//{
//    Console.WriteLine(debtor.FullName);
//    Console.WriteLine(DateTime.Today.Year - debtor.BirthDay.Year);
...
//var list = debtors.MaxBy(debtor => debtor.BirthDay.Year);
//Console.WriteLine(list!.BirthDay.Year);


//// 14

//var list = debtors.OrderBy(debtor => debtor.Debt).TakeLast(5).ToList();
//foreach (var debtor in list)
//{
...
//var DebtsSum = debtors.Sum(debtor => debtor.Debt);
//Console.WriteLine(DebtsSum);


//// 16

//var list = debtors.Where(debtor => 1939 <= debtor.BirthDay.Year && debtor.BirthDay.Year <= 1945);
...
//var CanPayDebtUntilBirthday = debtors.Where(db => (DateTime.Now.Month - db.BirthDay.Month) * 500 >= db.Debt);
//foreach (var debtor in CanPayDebtUntilBirthday)
//{
//    Console.WriteLine(debtor);
//    Console.WriteLine();
//}
...
//var list = debtors.Where(db => db.FullName.ToLower().Contains('s') && db.FullName.ToLower().Contains('m') && db.FullName.ToLower().Contains('i') && db.FullName.ToLower().Contains('l') && db.FullName.ToLower().Contains('e'));
//foreach (var debtor in list)
//{
//    Console.WriteLine(debtor);
//    Console.WriteLine();
